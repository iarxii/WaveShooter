...
- Enemy spawn pressure: added a configurable multiplier to reduce the number of basic enemies while preserving pacing.
...
- Expected FPS target: 53–60 on test hardware.
- With arena growth disabled and spawn pressure at 0.9, frame pacing should be noticeably smoother.
- Use the lightweight FPS meter for quick checks; enable Drei `Stats` only when diagnosing deeper issues.
...
- If wave density spikes, consider:
  ...
  - Spreading spawn bursts across a few frames or a short window per portal.
...
- `src/App.jsx`
  ...
  - FPS meter and Drei `Stats` toggles.
