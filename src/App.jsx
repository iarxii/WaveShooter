  ...
  useFrame((_, dt) => {
    ...
    if (isSpawning.current) {
      ...
      ref.current.position.y = Math.max(
        groundY,
        ref.current.position.y - DROP_SPEED * dt
      );
      ...
      if (dist > 0.6) {
        ...
        const ramp =
          settleTimer.current > 0
            ? 1 - Math.max(0, settleTimer.current - dt) / POST_LAND_SETTLE
            : 1;
    ...
    if (knockback.current.lengthSq() > 1e-6) {
      ...
      const decayRate =
        (isBoss ? KNOCKBACK_DECAY.boss : KNOCKBACK_DECAY.minion) * SPEED_SCALE;
...
function TriangleBoss({
  ...
  useFrame((_, dt) => {
    ...
    if (!stunned && isCharging.current) {
      ...
      if (chargeTimer.current < 1.5) {
        const chargeSpeed = Math.min(
          BOSS_SPEED * 3 * TRIANGLE_BOSS_SPEED_MULT,
          TRIANGLE_CHARGE_MAX
        );
        ref.current.position.addScaledVector(
          chargeDirection.current,
          chargeSpeed * dt * (speedScale || 1)
        );
      } else {
        // Stop charging
      ...
      if (circleDir.length() > 1) {
        ...
        const circleSpeed = Math.min(
          BOSS_SPEED * 1.2 * TRIANGLE_BOSS_SPEED_MULT,
          TRIANGLE_CIRCLE_MAX
        );
        ...
        ref.current.position.addScaledVector(
          circleDir,
          circleSpeed * slow * dt * (speedScale || 1)
        );
      }
    }
  ...
  return (
    <group>
      ...
      {/* Boss HP label */}
      <Text
        position={[pos[0], pos[1] + 3.6, pos[2]]}
        fontSize={0.45}
        color="#ffffff"
        anchorX="center"
        anchorY="bottom"
